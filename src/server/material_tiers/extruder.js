...
export const fixExtruderRecipeTier = (event) => {
    GTRegistries.MATERIALS.forEach((material) => {
        if (material.hasFlag(GTMaterialFlags.GENERATE_GEAR)) {
            event.remove({id: `gtceu:extruder/extrude_${material.name}_ingot_to_gear`});
        ...
        if (material.hasFlag(GTMaterialFlags.GENERATE_SMALL_GEAR)) {
            event.remove({id: `gtceu:extruder/extrude_${material.name}_ingot_to_small_gear`});
        ...
        if (material.hasFlag(GTMaterialFlags.GENERATE_RING)) {
            event.remove({id: `gtceu:extruder/extrude_${material.name}_ingot_to_ring`});
