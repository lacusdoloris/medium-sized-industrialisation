...
export const adjustMachineRecipesForTier = (event, tier) => {
    ...
    lazyShaped(
        `gtceu:${tier.name}_electric_furnace`,
        ["CZC", "ZHZ", "WZW"], 
       {Z: tier.heatingWire}
    ...
    lazyShaped(
        `gtceu:${tier.name}_alloy_smelter`,
        ["CZC", "ZHZ", "WZW"], 
    ...
    lazyShaped(
        `gtceu:${tier.name}_arc_furnace`,
        ["WTW", "CHC", "PPP"],
        {W: tier.quadrupleCable, T: "#forge:dusts/graphite", P: tier.primaryPlate}
    ...
    lazyShaped(
        `gtceu:${tier.name}_brewery`,
        ["GPG", "WHW", "CSC"],
        {
            P: `gtceu:${tier.name}_electric_pump`, S: tier.materials.heating.tagged("springs")
        }
    ...
    lazyShaped(
        `gtceu:${tier.name}_centrifuge`,
        ["CMC", "WHW", "CMC"],
        {},
    ).id(`nijika:auto/machines/${tier.name}/centrifuge`);

    // Chemical bath: Replace Glass, cable.
    event.remove({id: `gtceu:shaped/${tier.name}_chemical_bath`});
    lazyShaped(
        `gtceu:${tier.name}_chemical_bath`,
        ["VGW", "PGV", "CHC"],
        {P: `gtceu:${tier.name}_electric_pump`}
    ...
    lazyShaped(
        `gtceu:${tier.name}_chemical_reactor`,
        ["GRG", "WMW", "CHC"],
        {},
    ).id(`nijika:auto/machines/${tier.name}/chemical_reactor`);

    // Compressor: Replace wires.
    event.remove({id: `gtceu:shaped/${tier.name}_compressor`});
    lazyShaped(
        `gtceu:${tier.name}_compressor`,
        [" C ", "PHP", "WCW"],
        {},
    ).id(`nijika:auto/machines/${tier.name}/compressor`);
    ...
    lazyShaped(
        `gtceu:${tier.name}_forming_press`,
        ["WPW", "CHC", "WPW"],
        {}
    ).id(`nijika:auto/machines/${tier.name}/forming_press`);

    // Lathe: Replace cables, grinder.
    event.remove({id: `gtceu:shaped/${tier.name}_lathe`});
    lazyShaped(
        `gtceu:${tier.name}_lathe`,
        ["WCW", "MHZ", "CWP"],
        {Z: tier.materials.grinder},
    ).id(`nijika:auto/machines/${tier.name}/lathe`);

    // Mixer: Replace glass, rotor.
    event.remove({id: `gtceu:shaped/${tier.name}_mixer`});
    lazyShaped(
        `gtceu:${tier.name}_mixer`,
        ["GRG", "GMG", "CHC"],
        {}
    ).id(`nijika:auto/machines/${tier.name}/mixer`);

    // Ore Washer: Replace glass, rotor, cables.
    event.remove({id: `gtceu:shaped/${tier.name}_ore_washer`});
    lazyShaped(
        `gtceu:${tier.name}_ore_washer`,
        ["RGR", "CMC", "WHW"],
        {}
    ).id(`nijika:auto/machines/${tier.name}/ore_washer`);

    // Packer: Replace wires
    event.remove({id: `gtceu:shaped/${tier.name}_packer`});
    lazyShaped(
        `gtceu:${tier.name}_packer`,
        ["ZCZ", "RHV", "WCW"],
        {Z: "#forge:chests/wooden", R: `gtceu:${tier.name}_robot_arm`}
    ...
    lazyShaped(
        `gtceu:${tier.name}_polarizer`,
        ["ZMZ", "WHW", "ZMZ"],
        {Z: tier.materials.electricWire.component("double_wire"), M: tier.magneticRod}
    ...
    lazyShaped(
        `gtceu:${tier.name}_laser_engraver`,
        ["PEP", "CHC", "WCW"],
        {},
    ).id(`nijika:auto/machines/${tier.name}/laser_engraver`);

    // Sifter: Replace cables.
    event.remove({id: `gtceu:shaped/${tier.name}_sifter`});
    lazyShaped(
        `gtceu:${tier.name}_sifter`,
        ["WZW", "PHP", "CZC"],
        {Z: "gtceu:item_filter"}
    ).id(`nijika:auto/machines/${tier.name}/sifter`);

    // Thermal centrifuge: Replace wires.
    event.remove({id: `gtceu:shaped/${tier.name}_thermal_centrifuge`});
    lazyShaped(
        `gtceu:${tier.name}_thermal_centrifuge`,
        ["CMC", "ZHZ", "WMW"],
        {Z: tier.quadHeatingWire}
